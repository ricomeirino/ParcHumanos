...
public class CadastroAtividadesControl {
	...
	public Mensagem insereAtividade(Atividade atividadeNova, int projetoId) {
		...
		Atividade atividade = atividadeDAO.buscaAtividadePorCodigoMesmoProjeto(atividadeNova.getCodigo(), projetoId);
		...
		if (atividade != null) {
			...
			boolean inseriu = atividadeDAO.insertAtividade(atividadeNova, projetoId); 
					
			if (inseriu) {
				codigoMsg = 0;
				msg = "Atividade inserido com sucesso";
				//atividadeNova = atividadeDAO.buscaProjetoPorCodigoMesmoProjeto(atividadeNova.getCodigo(), projetoId); 
	...
	public Mensagem removeAtividade(Atividade atividadeRemovida) {
		...
		if (atividadeRemovida.isFinalizada()){
			...
			boolean recursoAlocado = alocacaoDAO.atividadeEstaAlocada(atividadeRemovida);
			...
			if (recursoAlocado){
				...
				// remover atividade
				boolean retorno = atividadeDAO.deleteAtividade(atividadeRemovida);
				if (retorno) {
					codigoMsg = 0;
	...
	public Mensagem atualizaAtividade(Atividade atividadeEditada) {
		...
		// testar se existe atividade com mesmo código no projeto
		Atividade atividade = atividadeDAO.buscaAtividadePorCodigoMesmoProjeto(atividadeEditada.getCodigo(), atividadeEditada.getProjeto().getIdProjeto());
		
		
		
		if (atividade == null || atividade.getIdAtividade() == atividadeEditada.getIdAtividade()) {
			// pode alterar
		...
		}else{ // se não for a mesma atividade -> não pode alterar pois o código já existe
			codigoMsg = 1;
			msg = "Atividade com código " + atividadeEditada.getCodigo()
					+ " já existe no projeto.";
		}			
